...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    for position in livingGhostPositionDistributions:
      maxPos = position.argMax()
      ghostDist[maxPos] = self.distancer.getDistance(pacmanPosition,maxPos)

    goal = getMin(ghostDist)

    #chooses an action
    for action in legal:
      successor = Actions.getSuccessor(pacmanPosition, action)
      actionDist[action] = self.distancer.getDistance(goal, successor)

    return getMin(actionDist)
